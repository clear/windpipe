...
export class Stream<T, E> extends Readable {
    /**
    ...
    static from<T, E>(value: Promise<T> | Iterator<T> | AsyncIterator<T> | Iterable<T> | AsyncIterable<T>): Stream<T, E> {
        if (value instanceof Promise) {
            // Likely a promise
            return Stream.fromPromise(value);
        }

        if ("next" in value && typeof value.next === "function") {
            // Likely an iterator
            return Stream.fromIterator(value);
        }
    ...
    static fromIterable<T, E>(iterable: Iterable<T> | AsyncIterable<T>): Stream<T, E> {
        if (Symbol.iterator in iterable) {
            return Stream.fromIterator(iterable[Symbol.iterator]());
        } else {
            return Stream.fromIterator(iterable[Symbol.asyncIterator]());
        }
    }
}
